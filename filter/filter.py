...
for i in range(episodes):
   ...
   # r = game.make_action(choice(actions))
    if POST_PROCESS:
        red = nc.image_postprocessing(game_state.image_buffer[0,:,:], IMAGE_SIZE, IMAGE_SIZE)
        green = nc.image_postprocessing(game_state.image_buffer[1,:,:], IMAGE_SIZE, IMAGE_SIZE)
        blue = nc.image_postprocessing(game_state.image_buffer[2,:,:], IMAGE_SIZE, IMAGE_SIZE)
    else:
...
ret,thresh1 = cv2.threshold(target,50,255,cv2.THRESH_BINARY)
ret,thresh2 = cv2.threshold(target,60,255,cv2.THRESH_BINARY_INV)
ret,thresh3 = cv2.threshold(target,60,255,cv2.THRESH_TRUNC)
ret,thresh4 = cv2.threshold(target,60,255,cv2.THRESH_TOZERO)
ret,thresh5 = cv2.threshold(target,60,255,cv2.THRESH_TOZERO_INV)
...
cv2.imwrite('tresh2.png',thresh2)
cv2.imwrite('tresh3.png',thresh3)
cv2.imwrite('tresh4.png',thresh4)
cv2.imwrite('tresh5.png',thresh5)
cv2.imwrite('Atresh1.png',th1)
cv2.imwrite('Atresh2.png',th2)
cv2.imwrite('Atresh3.png',th3)
cv2.imwrite('Atresh4.png',th4)
cv2.imwrite('Atresh5.png',th5)
cv2.imwrite('Atresh6.png',th6)